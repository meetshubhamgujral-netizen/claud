...
from modules import (
    AutoML, DataPreprocessor, DataProfiler, DescriptiveAnalytics,
    DiagnosticAnalytics, GeminiChatbot, Visualizer, comparison_table,
    confusion_matrix_fig, feature_importance_fig, roc_curve_fig,
)
from modules.utils import df_to_csv_bytes, human_format
...
def init_state() -> None:
    defaults = {
        ...
        "messages": [], "dark": True, "filename": None,
    }
...
@st.cache_data(show_spinner=False)
def analyse(_df: pd.DataFrame):
    """Profile + clean a dataframe. Cached on the dataframe hash."""
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if upload is not None and upload.name != st.session_state.filename:
            try:
                df = load_file(upload.getvalue(), upload.name)
    ...
    try:
        profile, clean, clean_profile, report = analyse(raw)
    except Exception as exc:  # noqa: BLE001
